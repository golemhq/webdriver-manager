...
class Test_webdriver_manager:
    ...
    def test_wm_without_args(self, test_utils):
        result = test_utils.run_command('webdriver-manager')
...
class Test_help:
    ...
    def test_help(self, command, expected, dir_session, test_utils):
...
class Test_udpate:
    ...
    @pytest.mark.slow
    def test_update_outputdir_does_not_exist(self, dir_function, test_utils):
        """Verify outputdir is created if it does not exist"""
        tempdir = dir_function['path']
        os.chdir(tempdir)
        command = 'webdriver-manager update -d chrome -o my_outputdir'
        result = test_utils.run_command(command)
    ...
    @pytest.mark.slow
    def test_update_version_specified_does_not_exist(self, dir_function, test_utils):
        tempdir = dir_function['path']
        os.chdir(tempdir)
        result = test_utils.run_command('webdriver-manager update -d chrome=99.99')
    ...
    def test_update_specified_driver_is_incorrect(self, dir_function, test_utils):
        tempdir = dir_function['path']
        os.chdir(tempdir)
        result = test_utils.run_command('webdriver-manager update -d invalid_driver')
